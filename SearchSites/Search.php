...
class Search {
    static public function searchByName($inputSiteName) {
        ...
        $sites = array();
        $areas = array();
        array($sites, $areas) = AccessForDataBase->selectByName($inputSiteName);
        //結果をsiteインスタンス配列とareaインスタンス配列を取得
        
        // CombinedSiteのインスタンス配列を作る
        $combinedSites = array();
        //siteインスタンス配列の要素文ループするfor文を書く
        for(int $i = 0; $i>count($sites); $i++){
        // for文内 : siteインスタンスのid_siteでimage,comment,categoryを検索し、インスタンス配列を受け取る
            $comment = AccessForDataBase_toGetMultipleData->selectCommentBySiteID($site[$i]->$id_site);
            $image = AccessForDataBase_toGetMultipleData->selectImageBySiteID($site[$i]->$id_site);
            $category = AccessForDataBase_toGetMultipleData->selectCategoryBySiteID($site[$i]->$id_site);
        // for文内 : site,area,image,comment,cartegoryでCombinedSiteインスタンスを作り、配列に格納
            $combinedSite = new CombinedSite($site[$i], $area[$i], $comment, $image, $category);
